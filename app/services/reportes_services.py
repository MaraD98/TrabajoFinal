...
from app.models.registro_models import Evento, TipoEvento

class ReporteService:

    # ---------------- ADMIN (Rol 1) ----------------
    @staticmethod
    def reportes_admin(db: Session):
        total_eventos = db.query(func.count(Evento.id_evento)).scalar()

        resultados_estado = db.query(Evento.id_estado, func.count(Evento.id_evento)).group_by(Evento.id_estado).all()
        eventos_por_estado = [{"estado": estado, "cantidad": cantidad} for estado, cantidad in resultados_estado]

        resultados_usuario = db.query(Evento.id_usuario, func.count(Evento.id_evento)).group_by(Evento.id_usuario).all()
   ...
   # ---------------- SUPERVISOR (Rol 2) ----------------
    @staticmethod
    def reportes_supervisor(db: Session, id_usuario: int):
        resultados_tipo = (
    ...
    def reportes_cliente(db: Session, id_usuario: int):
        # Placeholder: depende de tu modelo de inscripciones/notificaciones
        return {
